 ...
 * 第三个AsyncTask的使用：增加了进度条的进度显示，还有下载过程中的取消，
 * 这里为进度条窗口设置取消监听
...
public class MyDownLoadAsyncTask03 extends AsyncTask<String , Integer , byte[]> {
    private static final String TAG = "MyDownLoadAsyncTask03";
     ...
     * @param imageView
     */
    public MyDownLoadAsyncTask03(Context context , ImageView imageView){
        mContext = context;
    ...
    protected void onPreExecute() {
        ...
        pd.setButton(DialogInterface.BUTTON_NEGATIVE, "取消", new DialogInterface.OnClickListener() {
            ...
            public void onClick(DialogInterface dialog, int which) {
                ...
                cancel(true); //在cancel()方法设置为true,则会取消当前的AsyncTask
                //（1）注意去doInBackground里面去监听是否已经取消
                //（2）会调用onCancel()方法
            }
        });
    ...
    /**
     * 主线程：当触发cancel()方法时会调用此处，
     * 一旦调用onCancelled()方法，将不再执行onPostExecute()方法
     */
    @Override
    protected void onCancelled() {

        Log.d(TAG , mContext+"下载任务已经取消");
        //取消进度条窗口
        pd.dismiss();
    }
    ...
    protected byte[] doInBackground(String... params) {
        ...
        try {
            ...
            if(responseCode == 200){
                ...
                //读取输入流
                //isCancelled()方法用来判断是否已经取消下载任务了
                while((temp = ins.read(data)) != -1 && (!isCancelled())){
